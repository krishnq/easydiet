...
function addHotel() {
  $.blockUI({message: '<section class="mod model-2"><div class="spinner"></div>  </section>'});
    let hn = "" + document.getElementById("hotelName").value;
    let hpn = "+91" + document.getElementById("hotelPhno").value;
    let hloc = document.getElementById("location-select");
    let adrs = "" + document.getElementById("hotelAdress").value;
    let had = "" + hloc.options[hloc.selectedIndex].value;
    let bal = [];
    console.log(/^\d+$/.test(document.getElementById("hotelPhno").value));
    if (hn.length!==0 && hpn.length==13 && adrs.length!==0  && had.length!==0 && /^\d+$/.test(document.getElementById("hotelPhno").value)) {
        ...
        db.collection("clothType").get()
            .then(function (querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    ...
                    let temp = {};
                    temp["cost"] = doc.data().cost;
                    temp["clothtype"] = doc.data().type;
                    temp["balance"] = 0;
                    console.log(temp);
                    bal[doc.id] = temp;
                });
                db.collection("hotels").doc(hpn).set({
                    name: hn,
                    phno: hpn,
                    location: had,
                    address: adrs,
                    ...
                    console.log("Document successfully written!", bal);
                    $.unblockUI();
                    window.location = window.location.href;

                }).catch(function (error) {
                    $.unblockUI();
                    alertDialog("Error writing document: ", error);
                });
            });
    } else {
        $.unblockUI();
        let str = "no field should be empty";
        if(hpn.length!=13){
